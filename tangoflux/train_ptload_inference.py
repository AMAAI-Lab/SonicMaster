...
def parse_args():
    ...
    parser.add_argument(
        "--seed", type=int, default=27, help="A seed for reproducible training."
    )
    parser.add_argument(
        "--checkpointing_steps",
        type=str,
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Enable or disable inference of samples after each epoch during training.",
    )
    parser.add_argument(
        "--inference_during_training_file",
        type=str,
        default="",
        help="Path file for inference during training.",
    )
    parser.add_argument(
        "--text_cfg",
        type=bool,
        default=True,
        help="Dropping and replacing of prompts.",
    )
    parser.add_argument(
        "--audio_cfg",
        type=bool,
        default=True,
        help="Dropping of the audio condition input.",
    )
...
def main():
    ...
    optimizer_parameters = list(model.transformer.parameters()) + list(model.fc_text.parameters()) + list(model.fc_text_audio.parameters()) + list(model.audio_cond.parameters())
    ...
    generic_prompts=["Make it sound better!",
    "Can you improve the sound of this song?",
    "Improve this!",
    "Master this track for me, please."] # 10% chance to 
    ...
    for epoch in range(starting_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                if args.text_cfg:
                    for text_i, cur_text in enumerate(text):
                        text_dice=random.random()
                        if text_dice<0.1: #10% chance
                            text[text_i]=""
                        elif text_dice>0.9: #10% chance
                            text[text_i]=random.choice(generic_prompts) #5 prompts to choose
                        elif text_dice<0.5: #40/40 prompt1 or prompt2 from dataset
                            text[text_i]=alt_text[text_i]
                ...
                with torch.no_grad():
                    ...
                    # deg_audio_latent = unwrapped_vae.encode(
                    #     deg_audio_input
                    # ).latent_dist.sample()
                    # deg_audio_latent = deg_audio_latent.transpose(
                    #     1, 2
                    # )  ## Tranpose  to (bsz, seq_len, channel)

                # loss, _, _, _ = model(audio_latent, deg_audio_latent, text, duration=duration)
                loss, _, _, _ = model(audio_latent, deg_audio_latent, text, duration=duration)
        ...
        for step, batch in enumerate(eval_dataloader):
            with accelerator.accumulate(model) and torch.no_grad():
                ...
                #     1, 2
                # )  ## Tranpose  to (bsz, seq_len, channel)
                # deg_audio_latent = unwrapped_vae.encode(deg_audio_input).latent_dist.sample()
                # deg_audio_latent = deg_audio_latent.transpose(
                #     1, 2
                # )  ## Tranpose  to (bsz, seq_len, channel)

                val_loss, _, _, _ = model(audio_latent, deg_audio_latent, text, duration=duration)

                total_val_loss += val_loss.detach().float()
                eval_progress_bar.update(1)

        
        if args.inference_during_training and accelerator.is_main_process and (epoch + 1) % 10 == 0:
            infer_outputs = []
            ...
            for step, batch in enumerate(infer_dataloader):
                with torch.no_grad():
                    ...
                    inferred_result = unwrapped_model.inference_flow(
                        deg_audio_latent,
                        text,
            ...
            inference_output_dir = os.path.join(output_dir, "traininference_outputs_audiocond", f"epoch_{epoch + 1}")
        ...
        if args.inference_during_training and accelerator.is_main_process and (epoch + 1) % 10 == 0:
            ...
            inference_output_dir = os.path.join(output_dir, "traininference_outputs_noaudiocond", f"epoch_{epoch + 1}")
            os.makedirs(inference_output_dir, exist_ok=True)
